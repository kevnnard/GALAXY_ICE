...
const InventarioPage = () => {
	...
	return (
		<main className="flex-1 flex flex-col justify-center items-center w-full">
			<NuevoItemComponent router={router} />
						...
						<tbody className="">
							{ventas.map((row: any) => (
								<tr key={row.id} className="border border-neutral-700">
									...
									<td className="py-2">{row.sabor}</td>
									<td className="py-2">{row.cantidad}</td>
									<td className="py-2">{row.precio}</td>
									<td className="py-2">
										<ActualizarItemComponent row={row} router={router} />
									</td>
								</tr>
							))}
						</tbody>
